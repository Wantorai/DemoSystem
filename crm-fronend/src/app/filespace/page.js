...
export default function FileSpacePage() {
  ...
  const showContextMenu = (event, type, item, writable = canWrite) => {
    ...
    setContextMenu({ x: event.clientX, y: event.clientY, type, item, writable });
  };
    ...
    <main className="filespace-page min-h-[calc(100vh-64px)] bg-slate-50 p-4 md:p-8">
      <section className="mx-auto max-w-6xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex min-h-[560px]">
          <aside className="w-64 shrink-0 border-r border-slate-200 bg-slate-50/70 p-3">
          ...
          {loading ? <div className="flex flex-col items-center justify-center gap-3 py-20 text-slate-500"><LoaderCircle className="animate-spin text-blue-600" size={32} /><span>Загружаем содержимое…</span></div> : view === 'trash' ? (
            ...
            <div className="divide-y divide-slate-100">
              ...
              {items.folders.map((folder) => (
                <div key={folder.id} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === 'Enter') openFolder(folder); }} onClick={() => openFolder(folder)} onContextMenu={(event) => showContextMenu(event, 'folder', folder, view === 'search' ? folder.canWrite : canWrite)} className="flex w-full cursor-pointer items-center gap-3 px-3 py-3 text-left hover:bg-slate-50">
